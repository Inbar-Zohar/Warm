...
for ind_m=1:2
    ...
    % Sim
    % this_data_raw = zeros(size(this_data_raw));%meas.(sprintf('B_sim_%s',measurements{ind_m}));
...
if plot_sim; plot(time_s/factor, Theta_sim - mean(Theta_sim), 'color',linecolor(3)); end
xlim(time_s([1 end])/factor)
ylabel('\Theta (deg)')
