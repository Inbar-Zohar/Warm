...
PID_V_Bz = meas.PID_simR_out;
[srPSD_PID_Bz_V_srHz,~] = pwelch_and_smooth(PID_V_Bz,time_s,tmin,tmax);
...
plot(time_s/factor, PID_V_Bz - mean(PID_V_Bz), 'color',linecolor(4)); 
xlim(time_s([1 end])/factor)
ylabel('PID out (V)')
xlabel(xlbl)
...
plot(time_s/factor, movmean(PID_V_Bz,movmean_param)- mean(PID_V_Bz), 'color',brighten(linecolor(4),-0.3)); 
...
plot(f,srPSD_PID_Bz_V_srHz ,'LineWidth',2,'DisplayName','PID_{Bz}','Color',linecolor(4))
