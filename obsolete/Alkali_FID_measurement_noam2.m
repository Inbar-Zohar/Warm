...
% Coupling
curr_inst.setChCoupling(chnl, 'AC');
...
%Smoothing
span=10;
meas.FID_Rb_v_Smooth=smooth(meas.FID_Rb_v,span);

%Checking if there's cutting
All_Cutts=(abs(meas.FID_Rb_v)==curr_inst.getVscale).*meas.FID_Rb_v;
if norm(All_Cutts)==0
    Cutts=0;
else
    Cutts=1;
...
if Cutts==0
    ...
    c = fitresult.c; %[V]
    c_err = fitresult_int(3); %[V]

    %Computing chi squere 

    Delta1=yData-a85*sin(2*pi*f85*xData+phi85).*exp(-G85*xData)+a87*sin(2*pi*f87*xData+phi87).*exp(-G87*xData)+fitresult.c;
    chi1=Delta1'*Delta1;

    %Try another fit

    if FID_Good_Index==0
        meas.Next_Try_FID_T2_85 = T2_85; % sec
        meas.Next_Try_FID_T2_85_err = T2_85_err; % sec
        meas.Next_Try_FID_a85 = a85;
        meas.Next_Try_FID_a85_err = a85_err;
        meas.Next_Try_FID_T2_87 = T2_87; % sec
        meas.Next_Try_FID_T2_87_err = T2_87_err; % sec
        meas.Next_Try_FID_a87 = a87;
        meas.Next_Try_FID_a87_err = a87_err;
        meas.Next_Try_FID_f85 = f85;
        meas.Next_Try_FID_f85_err = f85_err;
        meas.Next_Try_FID_f87 = f87;
        meas.Next_Try_FID_f87_err = f87_err;
        meas.Next_Try_FID_phi85 = phi85;
        meas.Next_Try_FID_phi85_err = phi85_err;
        meas.Next_Try_FID_phi87 = phi87;
        meas.Next_Try_FID_phi87_err = phi87_err;
        meas.Next_Try_FID_g85=G85;
        meas.Next_Try_FID_g85_err=G85_err;
        meas.Next_Try_FID_g87=G87;
        meas.Next_Try_FID_g87_err=G87_err;
        meas.Next_Try_FID_c=c;
        meas.Next_Try_FID_c_err=c_err;
    else
        opts.Display = 'Off'; %(a85                   a87                    c                     f85                         f87                         g85                        g87                       phi85                          phi87                   x)
        opts.StartPoint =      [meas.Next_Try_FID_a85 meas.Next_Try_FID_a87  yData(end)             meas.Next_Try_FID_f85      meas.Next_Try_FID_f87       meas.Next_Try_FID_g85      meas.Next_Try_FID_g87     meas.Next_Try_FID_phi85        meas.Next_Try_FID_phi87   ];
        opts.Lower =       0.1*[meas.Next_Try_FID_a85 meas.Next_Try_FID_a87  -100*abs(yData(end))   9*meas.Next_Try_FID_f85    9*meas.Next_Try_FID_f87     meas.Next_Try_FID_g85      meas.Next_Try_FID_g87     0                              0                         ];
        opts.Upper =        10*[meas.Next_Try_FID_a85 meas.Next_Try_FID_a87  abs(yData(end))        0.11*meas.Next_Try_FID_f85 0.11*meas.Next_Try_FID_f87  meas.Next_Try_FID_g85      meas.Next_Try_FID_g87     0.5*pi                         0.5*pi                    ];
        [fitresult2, gof2] = fit(xData, yData, ft, opts);
        fitresult2_int = confint(fitresult2, erf(1 / sqrt(2)));
        fitresult2_int = diff(fitresult2_int,1)/2;
        Delta2=yData-fitresult2.a85*sin(2*pi*fitresult2.f85*xData+fitresult2.phi85).*exp(-fitresult2.g85*xData)+fitresult2.a87*sin(2*pi*fitresult2.f87*xData+fitresult2.phi87).*exp(-fitresult2.g87*xData)+fitresult2.c;
        chi2=Delta2'*Delta2;
    end
    %Choosing the best fit
    if FID_Good_Index~=0
       if chi2<chi1
            a85 = fitresult2.a85; %[V]
            a85_err = fitresult2_int(1);%[V]
            a87 = fitresult2.a87; %[V]
            a87_err = fitresult2_int(2);%[V]
            G85 = fitresult2.g85; %[Hz]
            G87 = fitresult2.g87; %[Hz]
            G85_err = fitresult2_int(6);    %[Hz]
            G87_err = fitresult2_int(7);    %[Hz]
            T2_85 = 1 / G85; %[sec]
            T2_85_err = G85_err / G85 .^ 2; % 
            T2_87 = 1 / G87; %[sec]
            T2_87_err = G87_err / G87 .^ 2; % 
            f85 = fitresult2.f85; %[Hz]
            f85_err = fitresult2_int(4);    %[Hz]
            f87 = fitresult2.f87; %[Hz]
            f87_err = fitresult2_int(5);    %[Hz]
            phi85 = fitresult2.phi85; %[rad / sec]
            phi85_err = fitresult2_int(8);  %[rad / sec]
            phi87 = fitresult2.phi87; %[rad / sec]
            phi87_err = fitresult2_int(9); %[rad / sec]
            c = fitresult2.c; %[V]
            c_err = fitresult2_int(3); %[V]
    ...
    meas.FID_c=c;
    meas.FID_c_err=c_err;

    FID_Good_Index=FID_Good_Index+1;
    logger.log('Alkali FID measurement done.')
    
else
    logger.log('Alkali FID measurement done- there was a cutting.')
end