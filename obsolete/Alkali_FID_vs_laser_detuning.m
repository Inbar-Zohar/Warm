...
meas.Rb_T2_85 = zeros(1, nn);
meas.Rb_T2_85_err = zeros(1, nn);
meas.Rb_a85 = zeros(1, nn);
meas.Rb_a85_err = zeros(1, nn);
meas.Rb_f85 = zeros(1, nn);
meas.Rb_f85_err = zeros(1, nn);
meas.Rb_phi85 = zeros(1, nn);
meas.Rb_phi85_err = zeros(1, nn);

% set empty vectors for the 87 results
meas.Rb_T2_87 = zeros(1, nn);
meas.Rb_T2_87_err = zeros(1, nn);
meas.Rb_a87 = zeros(1, nn);
meas.Rb_a87_err = zeros(1, nn);
meas.Rb_f87 = zeros(1, nn);
meas.Rb_f87_err = zeros(1, nn);
meas.Rb_phi87 = zeros(1, nn);
meas.Rb_phi87_err = zeros(1, nn);
...
for i1 = 1:nn
    ...
    if flg.FID_Rb_vs_Pump_And_Probe_detuning
        disp(['| meas #: (' num2str((i2 - 1) * nn + i1) ' / ' num2str(ss) ') | Set ' prm.which_laser ' laser detuning to ' num2str(meas.laser_det(i1)) ' volts. |'])
        logger.log(['| meas #: (' num2str((i2 - 1) * nn + i1) ' / ' num2str(ss) ') | Set ' prm.which_laser ' laser detuning to ' num2str(meas.laser_det(i1)) ' volts. |'])
    else
        disp(['| meas #: (' num2str(i1) ' / ' num2str(nn) ') | Set ' prm.which_laser ' laser detuning to ' num2str(meas.laser_det(i1)) ' volts. |'])
        logger.log(['| meas #: (' num2str(i1) ' / ' num2str(nn) ') | Set ' prm.which_laser ' laser detuning to ' num2str(meas.laser_det(i1)) ' volts. |'])
    end
...
if flg.plot_data
    ...
    figure
    subplot(211)
    plot(meas.laser_det, meas.Rb_T2_85, '.', 'markerSize', 8)
    legend('T_2', 'Location', 'Best');
    xlabel('Laser detuning (volts)')
    ylabel('T_2 (sec)')
    title(['Rubidium 85 T_2 vs ' prm.which_laser ' laser detuning'])
    subplot(212)
