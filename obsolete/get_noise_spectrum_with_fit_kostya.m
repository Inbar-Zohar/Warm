function [f, N, t, S, tau, computed_quantities] = get_noise_spectrum_with_fit_kostya(daq_data, V)
% The status is we started to work on the Allan calculation. The original
% function extracts the correct values. We need to complete this new
% function and compare to the old. Then contintue to PSD.
...
% dBz = 588e-6 * daq_data.Bz_V;                                   % [G] using 0.588mG/V calibrated by Roy on 11.04.21     
% dBz = 58.9764e-6 * daq_data.Bz_V;                               % [G] Corrected factor, considering the dBz current driver 15.08.22
%(^^^ R_coil = 1003 Ohm, current supply control = 1e-4 A/V,  ==> 588e-6(G/V) * 1003 (Ohm) * 1e-4 (A/V) ^^^)
dBz = 320e-6 * daq_data.Bz_V;                               % [G] using 0.32mG/V (3.2 G/A) calibrated by Yaron on 08.09.2022 
...
% Resample to the device's bandwidth
% Omega_deg_hr_rs = resample(Omega_deg_hr);

% fill outliers
Omega_deg_hr_fill_outliers = filloutliers(Omega_deg_hr,"linear","percentiles",[0.01 99.99]);
% figure(1), clf
% plot(t,Omega_deg_hr-Omega_deg_hr_fill_outliers)

% subtract mean and linear drift
p = polyfit(t,Omega_deg_hr_fill_outliers,1);
Omega_deg_hr_fill_outliers_polyval = Omega_deg_hr_fill_outliers-polyval(p,t);

m_avg_allan = 1:2:floor(numel(Omega_deg_hr_fill_outliers_polyval)/2);
[avar,tau1] = allanvar(Omega_deg_hr_fill_outliers_polyval,m_avg_allan,fs);
adev1 = sqrt(avar);
% figure(1), clf
% loglog(tau1,sqrt(avar),'.')
...
if sum(awn_bool) > 1
    ...
    % Extract physical quantities from fit (IEEE notation)
    allan_PHI_deg_srHz = 10^awn_allan_fit.b / sqrt(3) / sqrt(fit_bw);       % [deg/srHz]
    ...
    allan_PHI_deg_srHz = 10^awn_allan_fit.b / sqrt(3) / sqrt(fit_bw);           % [deg/srHz] (in Walker, the units of this term is deg)

    % Extract errors from fit (Walker notation) (needs to be double checked, I did this in a rush)
    allan_PHI_err = (10^(awn_allan_fit.b + awn_allan_fit_conf) - 10^(awn_allan_fit.b)) / sqrt(3) / sqrt(fit_bw);
