function [f, N, t, S, tau, computed_quantities] = get_noise_spectrum_with_fit(daq_data, V, fit_bw)
...
if V.american_lock
    ...
    W_Hz_129 = V.df129 / 5  * daq_data.Bz_V;      % [Hz]
...
%% Prepare our expected optimal curve
try
    % Typical values: at some point, should consider replacing with
    % up-to-date measured/extracted values
    ...
    % Magnetic sensitivity measurement data
    noise_floor_fTsqrtHz = 450;

    srPSD_vec_mdeg_sqrthr_func = @(f) get_expected_srPSD_mdeg_srhr_from_magnetic_noise(f,...
        Gamma129, Gamma131, kP, kI129, kI131, k_f, tau_f, noise_floor_fTsqrtHz, EMF129_uG, EMF131_uG);

catch
    srPSD_vec_mdeg_sqrthr_func = @(f) nan(size(f));
end
%% Plotting the Data
...
if isfield(daq_data,'DateTime_var')
    title(char(daq_data.DateTime_var))
end
...
plot(f, srPSD_vec_mdeg_sqrthr_func(f), ':', 'linew', 2, 'color', 0.5*ones(1,3),'DisplayName', 'Noise model');
